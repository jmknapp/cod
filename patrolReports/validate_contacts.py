#!/usr/bin/env python3
"""
Validate ship contacts data for plausibility.
...
2. Position sanity checks:
   - Latitude should be reasonable for Pacific theater (roughly 0-40°N, some S)
   - Longitude should be reasonable for Pacific theater (roughly 100°E - 180° - 160°W)
...
def validate_contacts():
    ...
    print("=" * 80)
    print("SHIP CONTACTS VALIDATION REPORT")
    print("=" * 80)
    ...
    if position_issues:
        for pi in position_issues:
            c = pi['contact']
            print(f"  P{c['patrol']} #{c['contact_no']:2d}: {pi['issue']} - {format_position(c)}")
    else:
    ...
    for patrol_num in sorted(patrols.keys()):
        ...
        for i in range(1, len(patrol_contacts)):
            ...
            # Check if contact numbers are sequential
            if curr['contact_no'] < prev['contact_no']:
